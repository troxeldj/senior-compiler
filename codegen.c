...
bool codegen_response_has_entity(struct response* response) {
	return codegen_response_acknowledged(response) && response->flags & RESPONSE_FLAG_RESOLVED_ENTITY && response->data.resolved_entity;
}
...
void codegen_stack_add(size_t stack_size) {
	if(stack_size != 0) {
		stackframe_add(current_function, STACK_FRAME_ELEMENT_TYPE_UNKNOWN, "stack_addition", stack_size);
...
void codegen_generate_number_node(struct node* node, struct history* history) {
	asm_push_ins_push_with_data("dword %i", STACK_FRAME_ELEMENT_TYPE_PUSHED_VALUE, "result_value", STACK_FRAME_ELEMENT_FLAG_IS_NUMERICAL, &(struct stack_frame_data){
...
void codegen_generate_scope_variable(struct node* node) {
	...
	if(node->var.value) {
		codegen_generate_expressionable(node->var.value, history_begin(EXPRESSION_IS_ASSIGNMENT | IS_RIGHT_OPERAND_OF_ASSIGNMENT));
		// pop eax
...
void codegen_generate_entity_access_start(struct resolver_result* result, struct resolver_entity* root_assignment_entity, struct history* history) {
	if(root_assignment_entity->type == RESOLVER_ENTITY_TYPE_UNSUPPORTED) {
		...
		asm_push_ins_push_with_data("dword [%s]", STACK_FRAME_ELEMENT_TYPE_PUSHED_VALUE, "result_value", 0, &(struct stack_frame_data){
		...
		asm_push_ins_push_with_data("ebx", STACK_FRAME_ELEMENT_TYPE_PUSHED_VALUE, "result_value", 0, &(struct stack_frame_data){
			.dtype=root_assignment_entity->dtype
		});
	}
}
...
bool codegen_resolve_node_for_value(struct node* node, struct history* history) {
	...
	#warning "more to go for resolving node's value"
	return true;
}
...
void codegen_generate_stack_scope(struct vector* statements, size_t size, struct history* history) {
...
void codegen_generate_function_arguments(struct vector* vec) {
	vector_set_peek_pointer(vec, 0);
	struct node* current = vector_peek_ptr(vec);
	while(current) {
		codegen_new_scope_entity(current, current->var.aoffset, RESOLVER_DEFAULT_ENTITY_FLAG_IS_LOCAL_STACK);
		current = vector_peek_ptr(vec);
	}
}
