...
void codegen_stack_add(size_t stack_size) {
  if (stack_size != 0) {
    stackframe_add(current_function, STACK_FRAME_ELEMENT_TYPE_UNKNOWN,
                   "stack_addition", stack_size);
    asm_push("add esp, %lld", stack_size);
  }
}
...
void codegen_goto_entry_point(struct node* node) {
  struct code_generator* generator = current_process->generator;
  struct codegen_entry_point* entry_point = codegen_current_entry_point();
...
void codegen_generate_variable_for_array(struct node* node) {
	if(node->var.value != NULL) {
		compiler_error(current_process, "Don't support values for arrays\n");
		return;
	}
	char tmp_buf[256];
	asm_push("%s: %s 0", node->var.name,
...
void codegen_generate_global_var_list(struct node* node) {
    ...
    case NODE_TYPE_VARIABLE:
      ...
      break;
		...
		case NODE_TYPE_VARIABLE_LIST:
			codegen_generate_global_var_list(node);
			break;
...
void codegen_generate_number_node(struct node* node, struct history* history) {
  asm_push_ins_push_with_data("dword %i", STACK_FRAME_ELEMENT_TYPE_PUSHED_VALUE,
                              "result_value",
                              STACK_FRAME_ELEMENT_FLAG_IS_NUMERICAL,
                              &(struct stack_frame_data){
                                  .dtype = datatype_for_numeric(),
                              },
                              node->llnum);
}
...
void codegen_generate_expression_paren_node(struct node* node, struct history* history) {
	codegen_generate_expressionable(node->parentheses.exp, history_down(history, codegen_remove_uninheritable_flags(history->flags)));
}
    ...
    case NODE_TYPE_EXPRESSION:
      ...
      break;
		...
		case NODE_TYPE_EXPRESSION_PARENTHESIS:
			codegen_generate_expression_paren_node(node, history);
			break;
...
void codegen_generate_scope_variable(struct node* node) {
  ...
  if (node->var.value) {
    codegen_generate_expressionable(
        node->var.value, history_begin(EXPRESSION_IS_ASSIGNMENT |
...
void codegen_generate_entity_access_start(
  ...
  if (root_assignment_entity->type == RESOLVER_ENTITY_TYPE_UNSUPPORTED) {
    ...
    asm_push_ins_push_with_data(
        "dword [%s]", STACK_FRAME_ELEMENT_TYPE_PUSHED_VALUE, "result_value", 0,
        &(struct stack_frame_data){
            .dtype = root_assignment_entity->dtype,
        },
        result->base.address);
...
void codegen_generate_entity_access_for_entity_for_assignment_left_operand(
  ...
  switch (entity->type) {
    ...
    default:
      compiler_error(current_process, "COMPILER BUG", entity->type);
  }
}
...
void codegen_generate_entity_access_for_array_bracket_pointer(struct resolver_result* result, struct resolver_entity* entity) {
	...
	if(datatype_element_size(&entity->dtype) > DATA_SIZE_WORD) {
...
void codegen_generate_entity_access_for_array_bracket(struct resolver_result* result,
																											struct resolver_entity* entity) {
...
bool asm_datatype_back(struct datatype* dtype_out) {
  ...
  if (!last_stack_frame_element) return false;
...
void codegen_setup_new_logical_expression(struct node* node,
                                          struct history* history) {
...
void codegen_generate_exp_node_for_logical_arithmetic(struct node* node,
                                                      struct history* history) {
  bool start_of_logical_expression =
      !(history->flags & EXPRESSION_IN_LOGICAL_EXPRESSION);
...
void codegen_generate_for_statement(struct node* node) {
	...
	if(for_stmt->condition_node) {
		codegen_generate_expressionable(for_stmt->condition_node, history_begin(0));
		asm_push_ins_pop_or_ignore("eax", STACK_FRAME_ELEMENT_TYPE_PUSHED_VALUE, "result_value");
		asm_push("cmp eax, 0");
...
void codegen_generate_switch_default_statement(struct node* node) {
...
void codegen_generate_label(struct node* node) {
	asm_push("label_%s:", node->stmt._goto.label->sval);
}
...
void codegen_generate_scope_variable_for_list(struct node* var_list_node) {
	...
	struct node* var_node = vector_peek(var_list_node->var_list.list);
	while (var_node) {
		codegen_generate_scope_variable(var_node);
		var_node = vector_peek(var_list_node->var_list.list);
	}
    ...
    case NODE_TYPE_EXPRESSION:
      ...
      break;
		...
		case NODE_TYPE_STATEMENT_DEFAULT:
			codegen_generate_switch_default_statement(node);
			break;
...
void codegen_generate_stack_scope(struct vector* statements, size_t size,
                                  struct history* history) {
