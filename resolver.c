...
struct datatype* resolver_get_datatype(struct resolver_process* resolver,
                                       struct node* node);
struct resolver_entity* resolver_follow_part_return_entity(
    struct resolver_process* resolver, struct node* node,
    struct resolver_result* result);
...
bool resolver_result_finsished(struct resolver_result* result) {
...
struct resolver_result* resolver_new_result(struct resolver_process* process) {
  ...
  result->array_data.array_entities =
...
struct resolver_scope* resolver_process_scope_current(
...
struct resolver_entity* resolver_result_peek_ignore_rule_entity(
    struct resolver_result* result) {
...
struct resolver_scope* resolver_scope_current(
    struct resolver_process* process) {
  return process->scopes.current;
}
...
struct resolver_scope* resolver_new_scope(struct resolver_process* resolver,
  ...
  resolver->scopes.current->next = scope;
  scope->prev = resolver->scopes.current;
  resolver->scopes.current = scope;
...
void resolver_finish_scope(struct resolver_process* resolver) {
  struct resolver_scope* scope = resolver->scopes.current;
  resolver->scopes.current = scope->prev;
...
struct resolver_entity* resolver_create_new_entity(
    struct resolver_result* result, int type, void* private) {
...
struct resolver_entity* resolver_create_new_entity_for_unsupported_nodes(
    struct resolver_result* result, struct node* node) {
...
struct resolver_entity* resolver_create_new_entity_for_merged_array_brackets(
    struct resolver_result* result, struct resolver_process* process,
    struct node* node, struct node* array_index_node, int index,
    struct datatype* dtype, void* private, struct resolver_scope* scope) {
  struct resolver_entity* entity = resolver_create_new_entity(
...
struct resolver_entity* resolver_create_new_unknown_entity(
...
struct resolver_entity* resolver_create_new_cast_entity(
    struct resolver_process* process, struct resolver_scope* scope,
...
struct resolver_entity* resolver_register_function(
  ...
  entity->name = function_node->func.name;
  entity->node = function_node;
  entity->dtype = function_node->func.rtype;
...
struct resolver_entity* resolver_get_entity_in_scope_with_entity_type(
  ...
  // check if we are accessing a structure or union
...
struct resolver_entity* resolver_get_entity_for_type(
    struct resolver_result* result, struct resolver_process* resolver,
...
struct resolver_entity* resolver_get_function(struct resolver_result* result,
  ...
  struct resolver_entity* entity = NULL;
  struct resolver_scope* scope = resolver->scopes.root;
  entity = resolver_get_function_in_scope(result, resolver, func_name, scope);
  return entity;
}

struct resolver_entity* resolver_follow_for_name(
    struct resolver_process* resolver, const char* name,
    struct resolver_result* result) {
  struct resolver_entity* entity =
...
struct resolver_entity* resolver_follow_identifier(
    struct resolver_process* resolver, struct node* node,
    struct resolver_result* result) {
  struct resolver_entity* entity =
...
struct resolver_entity* resolver_follow_variable(
    struct resolver_process* resolver, struct node* var_node,
    struct resolver_result* result) {
  struct resolver_entity* entity =
...
struct resolver_entity* resolver_follow_struct_exp(
    struct resolver_process* resolver, struct node* node,
    struct resolver_result* result) {
...
void resolver_build_function_call_arguments(
  ...
  if (is_argument_node(argument_node)) {
    ...
    resolver_build_function_call_arguments(
        resolver, argument_node->parentheses.exp, root_func_call_entity,
        total_size_out);
...
struct resolver_entity* resolver_follow_function_call(
...
struct resolver_entity* resolver_follow_parentheses(
    struct resolver_process* resolver, struct node* node,
    struct resolver_result* result) {
  if (node->exp.left->type == NODE_TYPE_IDENTIFIER) {
    return resolver_follow_function_call(resolver, result, node);
  }
  return resolver_follow_exp(resolver, node->parentheses.exp, result);
}
...
void resolver_array_bracket_set_flags(struct resolver_entity* bracket_entity,
  ...
  if (!(dtype->type & DATATYPE_FLAG_IS_ARRAY) ||
...
struct resolver_entity* resolver_follow_array_bracket(
    struct resolver_process* resolver, struct node* node,
    struct resolver_result* result) {
...
struct resolver_entity* resolver_follow_exp_parentheses(
    struct resolver_process* resolver, struct node* node,
    struct resolver_result* result) {
  return resolver_follow_part_return_entity(resolver, node->parentheses.exp,
                                            result);
}

struct resolver_entity* resolver_follow_unsupported_unary_node(
    struct resolver_process* resolver, struct node* node,
    struct resolver_result* result) {
...
struct resolver_entity* resolver_follow_unsupported_node(
    struct resolver_process* resolver, struct node* node,
    struct resolver_result* result) {
  bool followed = false;
  switch (node->type) {
  ...
  struct resolver_entity* unsupported_entity =
      resolver_create_new_entity_for_unsupported_nodes(result, node);
...
struct resolver_entity* resolver_follow_indirection(
    struct resolver_process* resolver, struct node* node,
    struct resolver_result* result) {
  ...
  struct resolver_entity* indirection_entity =
      resolver_create_new_unary_indirection_entity(
          resolver, result, node, node->unary.indirection.depth);
  resolver_result_entity_push(result, indirection_entity);
  return indirection_entity;
}

struct resolver_entity* resolver_follow_unary_address(
    struct resolver_process* resolver, struct node* node,
    struct resolver_result* result) {
  resolver_follow_part(resolver, node->unary.operand, result);
  struct resolver_entity* last_entity = resolver_result_peek(result);
...
struct resolver_entity* resolver_follow_part_return_entity(
  ...
  switch (node->type) {
    ...
    case NODE_TYPE_EXPRESSION_PARENTHESIS:
      entity = resolver_follow_exp_parentheses(resolver, node, result);
      break;
...
void resolver_execute_rules(struct resolver_process* resolver,
                            struct resolver_result* result) {
  struct vector* saved_entites = vector_create(sizeof(struct resolver_entity*));
  ...
  while (entity) {
    ...
    vector_push(saved_entites, &entity);
    last_processed_entity = entity;
    entity = resolver_result_pop(result);
  }
  resolver_push_vector_of_entities(result, saved_entites);
}
