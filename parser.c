...
struct parser_scope_entity {
  ...
  // the stack offset of scope entity.
  // negative for local. Positive for function argument scope
  int stack_offset;
...
struct history_cases {
  // A vector of parsed switch cases
  struct vector* cases;

  // is there a default keyword in the switch stmt body
...
static void expect_sym(char c) {
  ...
  if (!next_token || next_token->type != TOKEN_TYPE_SYMBOL ||
    ...
    compiler_error(
        current_process,
        "Expected symbol %c, however, something else was provided.\n", c);
  }
}
...
static void expect_op(const char* op) {
  ...
  if (!next_token || next_token->type != TOKEN_TYPE_OPERATOR ||
    ...
    compiler_error(
        current_process,
        "Expected operator %s, however, something else was provided.\n", op);
  }
}
...
static void expect_keyword(const char* keyword) {
  ...
  if(!next_token || next_token->type != TOKEN_TYPE_KEYWORD || !S_EQ(next_token->sval, keyword)) {
    compiler_error(current_process, "Expected the keyword %s, but something else was provided\n", keyword);
  }
}
...
static bool token_next_is_symbol(char c) {
  ...
  return token_is_symbol(next_token, c);
}
...
void parse_for_normal_unary() {
  ...
  struct node* unary_op_node = node_pop();
  make_unary_node(unary_op, unary_op_node);
}
...
void parse_exp_normal(struct history* history) {
  ...
  if (!node_left) {
    if(!parser_is_unary_operator(op)) {
      compiler_error(current_process, "The given expression doesn't have a left operand\n"); 
    }
    parse_for_unary();
    return;
  }
  ...
  // reorder the expression w/ associativity
...
void parse_get_datatype_tokens(struct token** datatype_token,
...
void parser_datatype_adjust_size_for_secondary(
  ...
  datatype += secondary_data_type->size;
...
void parser_datatype_init_type_and_size_for_primitive(
  ...
  if (!parser_datatype_is_secondary_allowed_for_type(datatype_token->sval) &&
    ...
    compiler_error(current_process,
                   "Your not allowed a secondary datat type here for the given "
                   "datatype %s\n",
                   datatype_token->sval);
  }
...
void parser_datatype_init_type_and_size(struct token* datatype_token,
  ...
  if (!parser_datatype_is_secondary_allowed(expected_type) &&
      datatype_secondary_token) {
    compiler_error(current_process,
                   "You provided an invalid secondary data type\n");
  }
...
void parser_datatype_init(struct token* datatype_token,
  ...
  if (S_EQ(datatype_token->sval, "long") && datatype_secondary_token &&
    ...
    compiler_warning(current_process,
                     "Our compiler does not support x64 bit longs. Therefore "
...
bool parser_is_int_valid_after_datatype(struct datatype* dtype) {
  return dtype && dtype->type == DATA_TYPE_LONG ||
         dtype->type == DATA_TYPE_FLOAT || dtype->type == DATA_TYPE_DOUBLE;
}
...
void parser_scope_offset_for_stack(struct node* node,
...
void make_variable_list_node(struct vector* list_vec) {
  node_create(&(struct node){.type = NODE_TYPE_VARIABLE_LIST,
                             .var_list.list = list_vec});
}
...
void parser_append_size_for_node(struct history* history, size_t* _variable_size,
                                 struct node* node);
...
void parser_append_size_for_node(struct history* history, size_t* _variable_size,
                                 struct node* node) {
    // can't do anything with a null node
    if(!node) return;
...
void parser_finalize_body(struct history* history, struct node* body_node,
                          ...
                          struct node* largest_align_eligable_var_node,
                          struct node* largest_possible_var_node) {
                            ...
                            *_variable_size += padding;
                            if(largest_align_eligable_var_node) {
                              *_variable_size = align_value(*_variable_size, largest_align_eligable_var_node->var.type.size);
                            }
                            bool padded = padding != 0;
                            body_node->body.largest_var_node = largest_align_eligable_var_node;
...
void parse_body_single_statement(size_t* variable_size, struct vector* body_vec,
  ...
  // Changes the variable size variable by the size of the statement node
...
void parse_body_multiple_statements(size_t* variable_size, struct vector* body_vec, struct history* history) {
  ...
  struct node* largest_align_eligable_var_node = NULL;

  // beginning of body, need to pop left curly 
  expect_sym('{');
  while(!token_next_is_symbol('}')) {
    parse_statement(history_down(history, history->flags));
    statement_node = node_pop();
    if(statement_node->type == NODE_TYPE_VARIABLE) {
      if(!largest_possible_var_node || (largest_possible_var_node->var.type.size <= statement_node->var.type.size)) {
        largest_possible_var_node = statement_node;
      }

      if(variable_node_is_primitive(statement_node)) {
        if(!largest_align_eligable_var_node || (largest_align_eligable_var_node->var.type.size <= statement_node->var.type.size)) {
          largest_align_eligable_var_node = statement_node;
        }
      }
    }
    // push statement node to body vec
    vector_push(body_vec, &statement_node);

    // Change var size if statement is a variable
    parser_append_size_for_node(history, variable_size, variable_node_or_list(statement_node));
  }
  // pop off right curly brace
  expect_sym('}');

  parser_finalize_body(history, body_node, body_vec, variable_size, largest_align_eligable_var_node, largest_possible_var_node);
  parser_current_body = body_node->binded.owner;
  // push body node back to stack
  node_push(body_node);
}
...
void parse_body(size_t* variable_size, struct history* history) {
  ...
  // parse a body with multiple statements between curly braces
...
void parse_union(struct datatype* dtype) {
  ...
  // if isn't forward declaration, then we must have a scope
  if (!is_forward_declaration) {
    parser_scope_new();
  }
...
void parse_struct_or_union(struct datatype* dtype) {
  switch (dtype->type) {
    ...
    default:
      compiler_error(
          current_process,
          "COMPILER BUG: The provided data type is not a struct or union.\n");
      break;
  }
}
...
void parse_variable_function_or_struct_union(struct history* history) {
  ...
  // ignore integer abbreviations if necessary i.e "long int" becomes "long"
  parser_ignore_int(&dtype);

  struct token* name_token = token_next();
  if (name_token->type != TOKEN_TYPE_IDENTIFIER) {
    compiler_error(current_process, "Expected an identifier after datatype\n");
  }
...
void parse_return(struct history* history) {
  ...
  // returns for no expressions (return type void)
  // i.e. return;
  if(token_next_is_symbol(';')) {
    expect_sym(';');
