...
bool expressionable_parser_left_op_has_priority(struct expressionable* expressionable, const char* left_op, const char* right_op) {
	...
	int precedence_left = expressionable_parser_get_precedence_for_operator(left_op, &left_group);
	int precedence_right = expressionable_parser_get_precedence_for_operator(right_op, &right_group);
...
void expressionable_parser_node_shift_children_left(struct expressionable* expressionable, void* node) {
	...
	const char* new_exp_op = expressionable_callbacks(expressionable)->get_node_operator(expressionable, node);
	expressionable_callbacks(expressionable)->make_expression_node(expressionable, new_exp_left_node, new_exp_right_node, new_exp_op);
...
void expressionable_parser_reorder_expression(struct expressionable* expressionable, void** node_out) {
	...
	if(left_node_type != EXPRESSIONABLE_GENERIC_TYPE_EXPRESSION && right_node && right_node_type == EXPRESSIONABLE_GENERIC_TYPE_EXPRESSION) {
		...
		if(expressionable_parser_left_op_has_priority(expressionable,main_op, right_op)) {
...
bool expressionable_generic_type_is_value_expressionable(int type) {
	return type == EXPRESSIONABLE_GENERIC_TYPE_NUMBER || type == EXPRESSIONABLE_GENERIC_TYPE_IDENTIFIER || type == EXPRESSIONABLE_GENERIC_TYPE_UNARY || type == EXPRESSIONABLE_GENERIC_TYPE_PARENTHESES || type == EXPRESSIONABLE_GENERIC_TYPE_EXPRESSION;
}
...
void expressionable_expect_op(struct expressionable* expressionable, const char* op) {
	...
	if(next_token == NULL || !token_is_operator(next_token, op)) {
		expressionable_error(expressionable, "Expected operator but something else was provided.\n");
	} else {
...
void expressionable_expect_sym(struct expressionable* expressionable, char c) {
	struct token* next_token = expressionable_token_next(expressionable);
	if(!next_token) {
		expressionable_error(expressionable, "Expecting symbol but no Symbol was provided.\n");
	} else if(!token_is_symbol(next_token, c)) {
		expressionable_error(expressionable, "Expecting symbol but something else was provided.\n");
	}
...
void expressionable_parse_parenthesis(struct expressionable* expressionable) {
	...
	if(left_node) {
		void* parenthesis_node = expressionable_node_pop(expressionable);
		expressionable_callbacks(expressionable)->make_expression_node(expressionable, left_node, parenthesis_node, "()");
	}
...
void expressionable_parse_for_unary_indirection(struct expressionable* expressionable) {
	int depth = expression_get_pointer_depth(expressionable);
	expressionable_parse(expressionable);
	void* unary_operand_node = expressionable_node_pop(expressionable);
...
void expressionable_parse_unary(struct expressionable* expressionable) {
	...
	if(op_is_indirection(unary_op)) {
		expressionable_parse_for_unary_indirection(expressionable);
		return;
	}
...
void expressionable_parse_for_operator(struct expressionable* expressionable) {
	...
	if(expressionable_peek_next(expressionable)->type == TOKEN_TYPE_OPERATOR) {
		if(S_EQ(expressionable_peek_next(expressionable)->sval, "(")) {
			expressionable_parse_parenthesis(expressionable);
